...
class YoutubeFollower():
    ...
    def clean_count(self, text_count):
        # Ignore non ascii
        utf8_count = text_count.encode('utf-8', 'ignore')
        # Ignore non numbers
        string_count = utf8_count.decode(encoding="utf-8")
        p = re.compile('[\d,]+')
        return int(p.findall(string_count)[0].replace(',', ''))
    ...
    def get_search_results(self, search_terms, max_results, top_rated=False):
        ...
        # Example https://www.youtube.com/results?sp=CAMSAhAB&q=%D0%B2%D0%B8%D0%B1%D0%BE%D1%80%D0%B8
        if self._gl:
            url = url + '&gl=' + self._gl
    ...
    def get_recommendations(self, video_id, nb_recos_wanted, depth, key):
        ...
        for watch_count in soup.findAll('div', {'class': 'watch-view-count'}):
            try:
                print ("watch count before clean is: " , watch_count)
                # watch count before clean is:  <div class="watch-view-count">1 594 перегляди</div>
